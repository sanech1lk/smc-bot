...
describe("stripQuery", () => {
  it("drops a password reset token from the URL", () => {
    expect(stripQuery("https://budchat.dev/reset-password?token=abc123")).toBe(
      "https://budchat.dev/reset-password"
    );
  });

  it("drops a fragment as well", () => {
    expect(stripQuery("https://budchat.dev/projects#secret")).toBe("https://budchat.dev/projects");
  });

  it("keeps the path, which is what makes an error locatable", () => {
    expect(stripQuery("https://budchat.dev/projects/abc/stages/def")).toBe(
      "https://budchat.dev/projects/abc/stages/def"
    );
  });
...
describe("scrubEvent", () => {
  it("removes the request body, cookies and query string", () => {
    const event = scrubEvent({
      request: {
        url: "https://budchat.dev/reset-password?token=abc",
    ...
    expect(event.request?.url).toBe("https://budchat.dev/reset-password");
  ...
  it("drops the server hostname", () => {
    const event = scrubEvent({ server_name: "budchat-prod-7f9c" });
