...
export async function POST(req: Request, { params }: { params: { id: string } }) {
  ...
  if (targetUser) {
    ...
    await sendMail({
      to: email,
      subject: `Kelma — вас добавили на объект «${project.name}»`,
      text: [
  ...
  await sendMail({
    to: email,
    subject: `Kelma — приглашение на объект «${project.name}»`,
    text: [
      "Здравствуйте!",
      "",
      `${user.name} приглашает вас на объект «${project.name}» (${project.address}) в Kelma —`,
