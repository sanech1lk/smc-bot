...
async function main() {
  ...
  const worker = await prisma.user.upsert({
    where: { email: "master@kelma.dev" },
    update: {},
    create: {
      email: "master@kelma.dev",
      name: "Дмитрий (мастер)",
  ...
  const client = await prisma.user.upsert({
    where: { email: "client@kelma.dev" },
    update: {},
    create: {
      email: "client@kelma.dev",
      name: "Анна (заказчик)",
  ...
  console.log(" - prorab@kelma.dev (admin)");
  console.log(" - master@kelma.dev (worker)");
  console.log(" - client@kelma.dev (client)");
}
