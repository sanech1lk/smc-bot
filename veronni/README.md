...
2. **Ключ Anthropic** — [console.anthropic.com](https://console.anthropic.com) → API Keys. Про деньги — [ниже](#сколько-стоит-один-заказ).
...
Платит только агент — база, админка и талоны бесплатны. Оценка ниже посчитана из размеров промпта (инструкции ≈1400 токенов, меню из 36 позиций ≈1100, описания инструментов ≈600) и типичного заказа в 5–6 сообщений клиента, то есть 8–10 обращений к модели. **Это расчёт, а не замер** — реальную цифру видно в консоли Anthropic после первых разговоров.

| Модель | Примерно за один полный заказ | На что хватит $5 |
| --- | --- | --- |
| `claude-opus-5` (по умолчанию) | 15–25 центов | ~25 заказов |
| `claude-sonnet-5` | 8–12 центов | ~50 заказов |
| `claude-haiku-4-5` | 3–4 цента | ~150 заказов |

Постоянная часть промпта кэшируется, поэтому со второго обращения в разговоре она стоит в десять раз дешевле — без этого счёт был бы примерно вдвое больше.

**Для демо оставьте Opus 5**: показывать надо лучшее, а два десятка разговоров стоят копейки. **Для боевой работы** имеет смысл перевести на Haiku и сравнить — заказ пиццы задача несложная, и разницы может не быть. Меняется одной строкой в `.env`:
